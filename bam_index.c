...
long long int getseqcount(char *file_name) 
{ 
    ...
    for (i = 0; i < idx->n; ++i) {
		...
		if (k != kh_end(h)){
			//printf("\t%llu\t%llu", (long long)kh_val(h, k).list[1].u, (long long)kh_val(h, k).list[1].v);
res+= (long long)kh_val(h, k).list[1].u + (long long)kh_val(h, k).list[1].v;
}
		//else printf("\t0\t0");
		//putchar('\n');
	}
	//printf("*\t0\t0\t%llu\n", (long long)idx->n_no_coor);
        res += (long long)idx->n_no_coor;
	bam_header_destroy(header);
	bam_index_destroy(idx);
	//return 0;
        return res; 
